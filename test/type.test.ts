...
import {
  ...
  type NullType,
  type NumberType,
  type UndefinedType} from './mock-data';
...
describe('Test linting types and registering objects types', () => {
  ...
  test('Object with children must pass if children key not exist in object', () => {
    const validEntryObjectWithChildren = Registrie<EntryObjectWithChildren>(
      'usage',
      'subCommands'
    );
    expect(() =>
      validEntryObjectWithChildren.register(mockEntryObjectWithoutChildren)
    ).not.toThrow();
  });
